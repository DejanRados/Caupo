...
namespace Caupo.Fiscal.Croatia
{
    public sealed class CroatiaInvoiceBuilder
    {
        ...
        public CroatiaInvoiceBuilder(
            CroatiaFiscalSettings settings)
        {
            _settings = settings;
            _taxCalculator =
                new CroatiaTaxCalculator(settings);
        }

        public async Task<CroatiaBuiltInvoice> BuildAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var taxes =
                await _taxCalculator.CalculateAsync(
                    request.Items,
                    cancellationToken);
            ...
            OznakaSlijednostiType sequence =
                string.Equals(
                    _settings.SequenceSetting,
                    "Poslovni prostor",
                    StringComparison.OrdinalIgnoreCase) ||
                string.Equals(
                    _settings.SequenceSetting,
                    "Na nivou poslovnog prostora",
                    StringComparison.OrdinalIgnoreCase)
                ...
                new RacunType
                {
                    BrRac =
                        new BrojRacunaType
                        {
                            ...
                            OznPosPr =
                                _settings.BusinessPremise,

                            OznNapUr =
                                _settings.CashRegister
                        },

                    DatVrijeme =
                        issueDateTime.ToString(
                            Fiscalization.DATE_FORMAT_LONG,
                            CultureInfo.InvariantCulture),
                    ...
                    Pnp =
                        consumptionTax.Length > 0
                            ? consumptionTax
                            : null,

                    USustPdv =
                        _settings.IsVatRegistered,

                    NacinPlac =
                        CroatiaPaymentMapper.ToCis(
                            request.PaymentType)
                };
