...
namespace Caupo.Fiscal.Croatia.Helpers
{
    public sealed class CroatiaTaxCalculator
    {
        ...
        public CroatiaTaxCalculator(CroatiaFiscalSettings settings)
        {
            _settings = settings;
        }

        public Task<IReadOnlyList<CroatiaTaxSummary>> CalculateAsync(
    IReadOnlyList<RacunStavka> items,
    CancellationToken cancellationToken = default)
        {
            return CalculateAsync(items, _settings.ConsumptionTaxRate, cancellationToken);
        }

        public async Task<IReadOnlyList<CroatiaTaxSummary>> CalculateAsync(
            IReadOnlyList<RacunStavka> items,
            decimal pnpRate,
            CancellationToken cancellationToken = default)
        {
            ...
            foreach (RacunStavka item in items)
            {
                ...
                if (item.PoreskaStopa.HasValue)
                {
                    if (!taxRates.TryGetValue(item.PoreskaStopa.Value, out vatRate))
                        throw new FiscalException($"Poreska stopa ID {item.PoreskaStopa.Value} za stavku '{item.Name}' nije pronađena u bazi.");
                }
            ...
            decimal taxAmount)
        {
            ...
            {
                totals[key] = (baseAmount, taxAmount);
            }
        }
