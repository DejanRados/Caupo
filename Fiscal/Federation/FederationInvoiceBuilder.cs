...
namespace Caupo.Fiscal.Federation
{
    public sealed class FederationInvoiceBuilder
    {
        ...
        public FederationBuiltInvoice BuildRefund(FiscalRequest request, int originalLocalReceiptNumber)
        {
            ...
            var invoice = new TringRacun();
            

            invoice.BrojRacuna = request.ReferentDocumentNumber;

            if (request.Buyer != null && !request.Buyer.IsCitizen)
            {
                invoice.Kupac = new TringKupac
                {
                    IDbroj = request.Buyer.TaxId ?? string.Empty,
                    Naziv = request.Buyer.Name ?? string.Empty,
                    Adresa = request.Buyer.Address ?? string.Empty,
                    Grad = request.Buyer.City ?? string.Empty
                };
            }
            ...
            foreach (Caupo.Models.RacunStavka item in request.Items)
            {
                ...
                if (quantity <= 0m)
                    throw new FiscalException($"Neispravna količina za stavku '{item.Name}'.");

                if (unitPrice < 0m)
                    throw new FiscalException($"Neispravna cijena za stavku '{item.Name}'.");

                var article = new TringArtikal
                {
                    Sifra = item.Sifra ?? string.Empty,
                    Naziv = item.Name ?? string.Empty,
                    JM = item.JedinicaMjereName,
                    Stopa = FederationTaxMapper.ToTring(item.PoreskaStopa),
                    Cijena = Convert.ToDouble(unitPrice)
                };

                var row = new TringRacunStavka
                {
                    artikal = article,
                    Kolicina = Convert.ToDouble(quantity),
                    Rabat = 0
                };

                invoice.DodajStavkuRacuna(row);
            }

            invoice.DodajVrstuPlacanja(
                FederationPaymentMapper.ToTring(request.PaymentType),
                0);
            ...
            {
                Invoice = invoice,
                LocalReceiptNumber = originalLocalReceiptNumber,
