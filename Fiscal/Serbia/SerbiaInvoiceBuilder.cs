...
namespace Caupo.Fiscal.Serbia
{
    public sealed class SerbiaInvoiceBuilder
    {
        ...
        public SerbiaInvoiceBuilder(
            SerbiaTaxMapper taxMapper)
        {
            _taxMapper = taxMapper;
        }

        public async Task<SerbiaInvoiceRequest>
            BuildAsync(
                ...
                CancellationToken cancellationToken = default)
        {
            ...
            foreach(var item in request.Items)
            {
                ...
                string label =
                    await _taxMapper.GetLabelAsync(
                        item.PoreskaStopa,
                        activeRates,
                        cancellationToken);
            ...
            {
                throw new FiscalException(
                    $"Kupac '{request.Buyer.Name}' " +
                    "nema poreski identifikacioni broj.");
            }
                ...
                Payment =
                    new List<SerbiaPayment>
                    {
                        new SerbiaPayment
                        {
                            ...
                            PaymentType =
                                SerbiaPaymentMapper.ToApiValue(
                                    request.PaymentType)
                        }
                    },
